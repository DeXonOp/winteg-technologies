...
async def submit_contact(data: ContactRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        print(f"  Company:  {data.company or 'N/A'}")
        print(f"  Phone:    {data.phone or 'N/A'}")
        print(f"  Service:  {data.service}")
        print(f"  Type:     {data.project_type or 'N/A'}")
        print(f"  Website:  {data.website or 'N/A'}")
        print(f"  Timeline: {data.timeline}")
        print(f"  Budget:   {data.budget or 'N/A'}")
        print(f"  Referral: {data.referral or 'N/A'}")
        print(f"  Tech:     {', '.join(data.technologies) if data.technologies else 'None'}")
